...
- Anthropic pptx-skill scripts removed from `main` (kept on the local branch
  `private/anthropic-pptx-scripts`). New own tools: `deck_edit.py` (list,
...
- **Rewriting git history now:** the private branch and earlier commits still
  hold the removed material; that is settled before any public visibility.
