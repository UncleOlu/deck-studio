...
class _CaseId(TypedDict):
    case: str
    deck: str | None


class CaseScore(_CaseId, total=False):
    """One report row; the QA fields are absent when the case delivered no deck."""

    register: str
    valid: bool
    storyline_errors: int
...
def score_case(case_dir: Path, case: str) -> CaseScore:
    ...
    out: CaseScore = {"case": case, "deck": deck.name, "register": reg}
