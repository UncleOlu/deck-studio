...
PLANTED = {
    "grocer-costout": ("FY25 revenue", "$4,820M @ store_pnl_FY25.xlsx!Summary!B3", "$4.61B @ cfo_email.txt"),
    "market-entry": (
        "2029 addressable market",
        "EUR 6.26B (formula) @ market_sizing.xlsx!Summary!C9",
        "EUR 7.4B @ partner_notes.md",
    ),
    "steerco-update": ("Finance go-live date", "2027-03 @ workstream_status.csv", "May 2027 @ pmo_weekly_note.md"),
    "take-private-valuation": (
        "Diluted shares outstanding",
        "58.2M @ capitalization.xlsx!Cap!B4",
        "57.4M @ management_projections.xlsx!Notes!A3",
    ),
    "sell-side-pitch": (
        "FY25 adjusted EBITDA",
        "$91M @ tidewater_financials.xlsx!P&L!F9",
    ...
    for i in range(1, 143):
        ...
        st.append(
            [
                f"HL{i:03d}",
                fmt,
                random.choice(["North", "Central", "Coastal"]),
                rev,
                round(rev * lab * random.uniform(0.9, 1.15)),
                round(rev * random.uniform(0.022, 0.036)),
                {"Superstore": 48000, "Neighbourhood": 22000, "Express": 7000}[fmt],
            ]
        )
    save(wb, d / "store_pnl_FY25.xlsx")
    write_csv(
        d / "peer_benchmarks.csv",
        ["peer", "labour_pct_sales", "shrink_pct_sales", "ebit_margin_pct"],
        [
            ["Peer A (public filings)", 10.4, 2.1, 4.9],
            ["Peer B (public filings)", 11.0, 2.4, 4.1],
            ["Peer C (industry survey)", 11.6, 2.6, 3.6],
            ["Median", 11.0, 2.4, 4.1],
        ],
    )
    write(
        d / "interview_notes.md",
        """
...
def market_entry(d: Path) -> None:
    ...
    write_csv(
        ...
        [
            ["Competitor North", "DE", 410, 620, 38],
            ["Competitor Atlas", "US", 260, 310, 34],
            ["Competitor Fjord", "NO", 150, 240, 41],
            ["Kestrel (US only today)", "US", 0, 0, 29],
        ],
    )
    write(
        d / "customer_interviews.md",
        """
...
def steerco(d: Path) -> None:
    write_csv(
        d / "workstream_status.csv",
        ["workstream", "owner", "status_rag", "pct_complete", "planned_go_live", "note"],
        [
            ["Finance (ERP GL/AP/AR)", "Controller", "Amber", 64, "2027-03", "Data migration mock 2 slipped 3 weeks"],
    ...
    for row in [
        ["Finance", 8200, 5900, 9100],
        ["Procurement", 3100, 2050, 3000],
        ["HR & Payroll", 4400, 1800, 5300],
        ["Supply chain", 5600, 2900, 5500],
        ["Reporting", 2300, 1100, 2900],
...
def take_private(d: Path) -> None:
    ...
    write_csv(
        d / "trading_comps.csv",
        ["company", "ev_usd_m", "ebitda_ltm_usd_m", "ebitda_ntm_usd_m"],
        [
            ["Peer Alder", 5120, 480, 505],
            ["Peer Birch", 2890, 301, 322],
            ["Peer Cedar", 7400, 690, 720],
            ["Peer Dogwood", 1980, 221, 230],
            ["Peer Elm", 3350, 305, 331],
        ],
    )
    write_csv(
        d / "precedent_transactions.csv",
        ["date", "target", "acquirer", "ev_usd_m", "ev_ltm_ebitda_x"],
        [
            ["2023-05", "Target One", "Sponsor A", 2100, 10.2],
            ["2024-02", "Target Two", "Strategic B", 3900, 11.8],
...
def sell_side(d: Path) -> None:
    ...
    write_csv(
        d / "buyer_universe.csv",
        ["buyer", "type", "rationale", "capacity_usd_bn"],
        [
    ...
    write_csv(
        d / "sector_multiples.csv",
        ["segment", "ev_ntm_ebitda_x_median", "n_deals_2023_2026"],
        [["Specialty foods (branded)", 13.1, 18], ["Private label food", 9.4, 11], ["Refrigerated", 12.2, 7]],
    )
    write(
        d / "owner_call_notes.md",
        """
