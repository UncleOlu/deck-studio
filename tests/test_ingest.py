...
EVALS = ingest.Path(__file__).resolve().parents[1] / "evals"
...
def test_planted_conflict_found_in_each_fixture(tmp_path, case, needle):
...
def test_independent_conflict_and_non_conflict(tmp_path):
    src = tmp_path / "src"
    src.mkdir()
    wb = Workbook()
    ws = wb.active
...
def test_circular_reference_does_not_hang(tmp_path):
    src = tmp_path / "src"
    src.mkdir()
    wb = Workbook()
    ws = wb.active
...
def test_symlinks_hidden_and_oversize_files_are_not_read(tmp_path):
    src = tmp_path / "src"
    src.mkdir()
...
def test_output_inside_input_is_refused(tmp_path):
    src = tmp_path / "src"
    src.mkdir()
    assert ingest.main([str(src), "--out", str(src / "brief")]) == 2


def test_symlinked_input_folder_is_refused(tmp_path):
    real = tmp_path / "real"
    real.mkdir()
...
def test_integer_followed_by_a_word_is_kept():
...
def test_zip_bomb_part_and_corrupt_pdf_land_in_not_read(tmp_path, monkeypatch):
    import zipfile
    src = tmp_path / "src"
    src.mkdir()
