...
SCALE_WORDS = {"k": 1e3, "thousand": 1e3, "m": 1e6, "mm": 1e6, "mn": 1e6, "million": 1e6,
               "b": 1e9, "bn": 1e9, "billion": 1e9}
CURRENCY = {"$": "USD", "usd": "USD", "€": "EUR", "eur": "EUR", "£": "GBP", "gbp": "GBP"}
STOP = set("""a an the of to in on for and or is was were be been are at by with from as that this it its
...
SYNONYMS = {"sales": "revenue", "turnover": "revenue", "tam": "market", "addressable": "market",
            "golive": "go-live", "go": "go-live", "live": "go-live", "headcount": "employees"}
...
NUM_RE = re.compile(
    ...
    re.I)
DATE_RE = re.compile(r"\b(?:(?P<y1>20\d{2})-(?P<m1>0[1-9]|1[0-2])\b"
                     r"|(?P<mon>jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(?P<y2>20\d{2}))", re.I)
...
class Formula:
    ...
    TOKEN = re.compile(r"\s*(?:(?P<num>\d+(?:\.\d+)?)|(?P<ref>\$?[A-Z]{1,3}\$?\d+(?::\$?[A-Z]{1,3}\$?\d+)?)"
                       r"|(?P<fn>SUM)\(|(?P<op>[-+*/(),]))")

    def __init__(self, expr: str, lookup):
        self.tokens, pos = [], 0
...
def _zread(z: zipfile.ZipFile, name: str) -> bytes:
    ...
    if total > MAX_FILE_BYTES:  # many parts under the per-part cap can still add up
        raise ValueError(f"the file decompresses to {total // (1024 * 1024)} MB, over the "
                         f"{MAX_FILE_BYTES // (1024 * 1024)} MB limit")
    size = z.getinfo(name).file_size
    if size > MAX_MEMBER_BYTES:
        raise ValueError(f"{name} decompresses to {size // (1024 * 1024)} MB, over the "
...
def evaluate_formulas(cells):
    grid = {(s, r): v for s, r, v, _ in cells}
...
def xlsx_facts(path: Path, rel: str, new_id) -> tuple[str, list[Fact]]:
    ...
    for sheet, grid in by_sheet.items():
        ...
        for ref, (v, f) in grid.items():
            ...
            if f:
                text += f" [formula {f}]"
            facts.append(Fact(new_id(), round(v, 6), unit, sc,
                              period_of(header) or period_of(label) or sheet_period,
                              text[:200], rel, f"{sheet}!{ref}"))
    return summary, facts


def csv_facts(path: Path, rel: str, new_id) -> tuple[str, list[Fact]]:
    ...
    for r_i, row in enumerate(rows[1:], start=2):
        ...
        for c_i, cell in enumerate(row[1:], start=1):
            ...
            if ds and re.fullmatch(r"\s*20\d{2}-\d{2}\s*", cell):
                facts.append(Fact(new_id(), ds[0][0], "date", 1.0, "", f"{label} | {h}"[:200], rel,
                                  f"row {r_i}, {h}"))
                continue
            ...
            if hl.endswith("_x") or "_x_" in hl:
                unit = "x"
            facts.append(Fact(new_id(), v, unit, sc, period_of(h) or period_of(label), f"{label} | {h}"[:200],
                              rel, f"row {r_i}, {h}"))
    return f"{len(rows) - 1} rows × {len(header)} columns: {', '.join(header)[:160]}", facts


def prose_facts(text: str, rel: str, new_id, locator_prefix: str = "line") -> list[Fact]:
    facts = []
    for ln, line in enumerate(text.splitlines(), 1):
        ...
        for v, unit, sc, s, e in numbers_in(line):
            if any(s < te and e > ts for ts, te in taken):
                continue
            facts.append(Fact(new_id(), v, unit, sc, period_of(window(line, s, e, 5, 3)),
                              window(line, s, e)[:200], rel, f"{locator_prefix} {ln}"))
    return facts
...
def same_at_precision(a: Fact, b: Fact) -> bool:
    ...
    step = max(10 ** -decimals(float(a.value)) * a.scale, 10 ** -decimals(float(b.value)) * b.scale)
...
def find_conflicts(facts: list[Fact]) -> list[tuple[Fact, Fact, set]]:
    out, seen = [], set()
    ...
    for a, b, shared in out:
        ...
        if dated is not None:
            ...
            siblings = [f for f in facts if f.file == dated.file and f.locator.split("!")[0] == sheet
                        and match_words(f) & shared and f.period]
...
def main(argv: list[str] | None = None) -> int:
    ...
    for p in files:
        ...
        except (zipfile.BadZipFile, KeyError, ET.ParseError, ValueError, UnicodeDecodeError, IndexError,
                AttributeError, *pdf_errors()) as err:  # one bad file lands in "Not read"; it never stops the run
    ...
    for a, b, shared in conflicts:
        lines.append(f"- **{', '.join(sorted(shared))}**: {fmt(a)} ({a.id}, {a.file} {a.locator}) vs "
                     f"{fmt(b)} ({b.id}, {b.file} {b.locator})")
        lines.append(f"  - {a.id}: {a.text}")
        lines.append(f"  - {b.id}: {b.text}")
    (out / "conflicts.md").write_text("\n".join(lines) + "\n")
    print(f"{len(files)} files, {len(facts)} facts, {len(conflicts)} possible conflicts, "
          f"{len(skipped)} not read → {out}")
    return 0
