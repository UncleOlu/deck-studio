...
REGISTERS: dict[str, Register] = {
    "keynote": {"title_max_words": 25, "exec_by": 0, "source_on_slide": False, "assertion": True, "unit_line": False},
    # R1 action titles, R2 <= 20 words (client p90), R4 exec summary by slide 3, R5 on-slide sources
    "consulting": {"title_max_words": 20, "exec_by": 3, "source_on_slide": True, "assertion": True, "unit_line": False},
    # R3 neutral topic labels allowed, <= 12 words (banking p95 = 11); R5 sources; R7 unit line (warning)
    "banking": {"title_max_words": 12, "exec_by": 3, "source_on_slide": True, "assertion": False, "unit_line": True},
}
UNIT_LINE = re.compile(
    r"^\s*\((?=[^)]*(?:[$€£%]|US\$|USD|EUR|GBP|in\s+(?:millions|billions|thousands)|per\s+share))"
    r"[^)]*\)(?:\s*\(\d+\))*\s*$",
    re.I | re.M,
)  # a trailing note marker "(1)" is allowed
MIN_WORDS = 5
...
EXEMPT_TITLE = re.compile(
    r"^(agenda|contents|table of contents|appendix|disclaimer|important (information|notice)|"
    r"notice|thank you|questions|q&a|next steps)\b",
    re.I,
)
EXEC_TITLE = re.compile(
    r"executive summary|summary of (?:valuation )?(analyses|findings|recommendations)"
    r"|valuation summary"
    r"|key (findings|takeaways|messages|considerations|questions)|^summary\b|recommendation",
    re.I,
)
# Narrow on purpose: "pending" or "to be confirmed" alone is normal SteerCo language, not a gap page.
GAP_TITLE = re.compile(
    r"\b(analys[ie]s pending|pending (further )?(data|information)|not performed"
    r"|open data items|data (gaps|requests?)|information (requests?|needed))\b",
    re.I,
)
...
MISSING_DATA = re.compile(
    r"\b(data|figures?|numbers|inputs?|information|analys[ie]s)\b[^.\n]{0,40}?"
    ...
    r"|\bpending (further )?(data|information|figures)\b",
    re.I,
)
...
SELF_REF = re.compile(
    r"\b(the|this|our) (input )?(file|files|workbook|spreadsheet|folder|inputs?)\b"
    ...
    r"|\bnot (provided|supplied) in the (materials|inputs?|files?)\b",
    re.I,
)
...
def lint(path: str, register: str) -> tuple[list[str], list[str], list[str]]:
    ...
    exec_at = 0
    front = 0
    tracker_seq: list[tuple[int, str]] = []
    for s in deck.slides:
        ...
        if (
            ...
            and not in_appendix
            and (
                EXEC_TITLE.search(s.title)
                or (
                    s.index - front <= max(cfg["exec_by"], 3) and not title_problem(s.title, 99) and claim_lines(s) >= 3
                )
            )
        ):
        ...
        if register != "keynote" and not legal:
            text = f"{s.title}\n{s.body_text}"
            self_ref = SELF_REF.search(text)
            if self_ref:
                hit = self_ref.group(0)
            ...
            if not in_appendix and (GAP_TITLE.search(s.title) or GAP_BODY.search(s.body_text) or soft_gap):
                errors.append(
                    f"ERROR slide {s.index}: data-gap page in the main story — answer with what the data "
    ...
    if cfg["exec_by"]:
        first_content = next(
            (s for s in deck.slides[1:] if not EXEMPT_TITLE.match(s.title) and not is_divider(s)), None
        )
        # An answer-first slide 2 counts: the plan's standard order puts the recommendation there.
        if (
            not exec_at
            and first_content is not None
            and first_content.index <= cfg["exec_by"]
            and (
                re.search(r"recommend|should|we propose|the answer", first_content.title, re.I)
                or first_content.title.split(" ", 1)[0].lower() in BASE_VERBS
            )
        ):  # an imperative is an answer
            exec_at = first_content.index
        if not exec_at or exec_at > cfg["exec_by"]:
            where = f"slide {exec_at}" if exec_at else "nowhere"
            errors.append(
                f"ERROR deck: executive summary must sit at or before slide {cfg['exec_by']} (found: {where})"
            )
    if tracker_seq:
        order: list[str] = []
        last = -1
