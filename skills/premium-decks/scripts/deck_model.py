...
class Chart:
    ...
    categories: list[str]
    series: list[dict[str, Any]]  # {name, values, invisible}
    html: bool = False
...
def _text_of(tx: Any) -> tuple[str, float, list[str], list[str]]:
...
def _text_detail(
    tx: Any,
) -> tuple[str, float, list[str], list[str], list[float], list[tuple[float, int]]]:
    ...
    return (
        ...
        sups,
        [paras[i] for i in keep],
        [para_pts[i] for i in keep],
        [para_em[i] for i in keep],
    )


def _geometry(el: Any) -> tuple[float, float, float, float]:
...
def _chart(z: zipfile.ZipFile, part: str) -> Chart | None:
    ...
    for el in plot:
        ...
        for ser in el.findall("c:ser", NS):
            ...
            invisible = sppr is not None and (
                sppr.find("a:noFill", NS) is not None or (fill is not None and fill.get("val", "").upper() == "FFFFFF")
            )
            series.append({"name": name_el.text if name_el is not None else "", "values": vals, "invisible": invisible})
                ...
                if tag == "sp":
                    ...
                    if tbl is not None:
                        ...
                        for tr in tbl.findall("a:tr", NS):
                            rows.append(
                                [
                                    _text_of(tc.find("a:txBody", NS))[0] if tc.find("a:txBody", NS) is not None else ""
...
def _pick_tracker(slide: Slide) -> str:
    ...
    for s in slide.shapes:
        if (
            ...
            and s is not title
            and s.text.strip()
            and s.y + s.h <= title.y + 0.02
            and len(s.text.split()) <= 12
            and not re.fullmatch(r"\s*\d+\s*", s.text)
        ):
            return s.text.strip()
    return ""
...
class _HtmlDeck(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag in self.VOID:
            if self.cur and tag == "img":
                ...
                self.cur.shapes.append(Shape("img", "picture", alt=a.get("alt") or ""))
            if tag == "br":
                self.buf.append("\n")
        ...
        if self.wf_depth and self.depth == self.wf_depth:
            ...
            self.cur.charts.append(
                Chart(
                    ...
                    bar_dir="col",
                    problems=[
                        f"waterfall bar '{b}': cannot read a number from --from/--to "
                        "(write plain numbers, not calc() or var())"
                        for b in self.wf_bad
                    ]
    ...
    def _close(self) -> None:
        ...
        s = self.cur
        assert s is not None  # only called from handle_endtag, which returns early when no slide is open
