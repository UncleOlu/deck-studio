...
def pptx_text(path: str) -> list[tuple[str, str]]:
    out: list[tuple[str, str]] = []
    with zipfile.ZipFile(path) as z:
        slides = sorted(n for n in z.namelist() if re.match(r"ppt/slides/slide\d+\.xml$", n))
        for name in slides:
            xml = z.read(name).decode("utf-8", errors="replace")
            num_m = re.search(r"slide(\d+)", name)
            assert num_m is not None  # name matched ppt/slides/slide\d+\.xml above
            num = num_m.group(1)
...
def html_text(path: str) -> list[tuple[str, str]]:
...
def main() -> int:
    ...
    print(
        f"\n{hits} finding(s) across {len(blocks)} text block(s)."
        if hits
