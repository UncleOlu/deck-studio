...
def format_result(result: dict[str, Any], domain: str) -> str:
    ...
    elif domain == "archetype":
        output.append(f"**{result.get('archetype', 'N/A')}** ({result.get('registers', '')})")
        output.append(f"  Answers: {result.get('answers_question', 'N/A')}")
        output.append(f"  Required: {result.get('required_elements', 'N/A')}")
        output.append(f"  Build with: {result.get('build_with', 'N/A')}")
        output.append(f"  Avoid when: {result.get('avoid_when', 'N/A')}")
        output.append(
            f"  Corpus share: MBB client {result.get('share_mbb_client_pct', '?')}% · "
            f"banking {result.get('share_banking_pct', '?')}%"
        )
...
Contextual Search (Premium System):
  ...
  search-slides.py "cta" --context --position 9 --total 9 --prev-emotion frustration
    ...
    parser.add_argument(
        "-d", "--domain", choices=AVAILABLE_DOMAINS, help="Specific domain to search (auto-detected if not specified)"
    )
    parser.add_argument("-n", "--max-results", type=int, default=3, help="Maximum results to return (default: 3)")
    parser.add_argument("--all", action="store_true", help="Search across all domains")
    parser.add_argument("--json", action="store_true", help="Output as JSON")

    # Contextual search options
    parser.add_argument(
        "--context", action="store_true", help="Use contextual search with layout/typography/color recommendations"
    )
    parser.add_argument("--position", type=int, default=1, help="Slide position in deck (1-based, default: 1)")
    parser.add_argument("--total", type=int, default=9, help="Total slides in deck (default: 9)")
    parser.add_argument(
        "--prev-emotion", type=str, default=None, help="Previous slide's emotion for contrast calculation"
    )
    ...
    if args.context:
        result = search_with_context(
            args.query, slide_position=args.position, total_slides=args.total, previous_emotion=args.prev_emotion
        )
