...
def _check(prs, *numbers: int) -> None:
    n = len(prs.slides)
    for k in numbers:
        if not 1 <= k <= n:
...
def _copy_chart(src_part: ChartPart, package) -> ChartPart:
    ...
    new = ChartPart.load(partname, src_part.content_type, package, src_part.blob)
...
def duplicate(prs, n: int, after: int | None) -> int:
    _check(prs, n)
    src = prs.slides[n - 1]
...
def delete(prs, numbers: list[int]) -> None:
    _check(prs, *numbers)
    ids = _slide_ids(prs)
    entries = list(ids)
...
def move(prs, n: int, to: int) -> None:
    _check(prs, n, to)
    ids = _slide_ids(prs)
    entry = list(ids)[n - 1]
    ids.remove(entry)
...
def listing(path: Path) -> list[str]:
    ...
    return [f"{s.index:>3}  [{s.layout or '-'}]  {s.title.splitlines()[0][:80] if s.title else ''}"
