...
def slide_numbers(slide) -> list[tuple[str, ingest.Fact]]:
    ...
    for block in slide.texts:
        ...
        for line in block.splitlines():
            ...
            line = re.sub(r"\b\d{1,2}(?=\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4})",
                          "", line)  # the day in "15 May 2026" belongs to a date
            if re.fullmatch(r"\d{1,3}", stripped):
                continue  # a lone small integer: page number or footnote marker
            if SECTION_STRIP.fullmatch(stripped) and re.search(r"(?:^|·)\s*\d{1,2}\s+[A-Z]", stripped) and (
                    "·" in stripped or re.match(r"\d{1,2}\s{2,}[A-Z]", stripped)):
    ...
    for s in deck.slides:
        ...
        for raw, num in slide_numbers(s):
            ...
            if digits and any(digits in re.sub(r"[^\d.\s]", " ", ln).split() or raw in ln for ln in calc_lines):
                continue
            if any(abs(v - num.value) < 1e-9 for v in illus):
                continue
