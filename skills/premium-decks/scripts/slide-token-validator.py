...
class Decl:
    ...
    def __init__(self, prop: str, val: str, line: int) -> None:
        self.prop, self.val, self.line = prop, val, line


def split_decls(body: str) -> list[Decl]:
    ...
    for piece in body.split(";"):
        if ":" in piece:
            ...
            out.append(Decl(prop.strip().lower(), val.strip(), body.count("\n", 0, pos + piece.index(":"))))
        pos += len(piece) + 1
    return out


def declaration_defects(prop: str, val: str) -> Iterator[str]:
...
def strip_root_and_print(css: str) -> str:
    ...
    out, i, n = [], 0, len(css)
    while i < n:
        m = re.compile(r"(^|[\s}])(:root[^{;]*|@media\s+print[^{;]*)\{", re.M).search(css, i)
        if not m:
            out.append(css[i:])
            break
...
def check(path: Path) -> int:
    ...
    for m in STYLE_ATTR.finditer(text):
        ...
        for d in m.group(1).split(";"):
            ...
            if not val.strip():
                continue
            findings.append(
                (line, f"inline style '{d.strip()}' — {prop.strip()} is not layout data; use a var(--token)")
            )
    ...
    for m in PRES_ATTR.finditer(text):
        ...
        if name == "font-size":
            ...
            bad = bool(HEX.search(val) or COLOR_FN.search(val) or re.match(r"^[A-Za-z][\w \-,\'\"]*$", val))
        if bad:
            findings.append(
                (
                    text.count("\n", 0, m.start()) + 1,
                    f'{name}="{val}" — var() support in SVG presentation attributes is uneven; move it to a CSS class',
                )
            )
