...
CSV_CONFIG: dict[str, _CsvConfig] = {
    ...
    "archetype": {
        "file": "slide-archetypes.csv",
        "search_cols": ["archetype", "registers", "keywords", "answers_question"],
        "output_cols": [
            "archetype",
            "registers",
            "answers_question",
...
def search_all(query: str, max_results: int = 2) -> dict[str, dict[str, Any]]:
...
def _load_decision_csv(csv_type: str) -> dict[str, dict[str, str]]:
...
def get_layout_for_goal(goal: str, previous_emotion: str | None = None) -> dict[str, Any]:
    """
...
def get_typography_for_slide(slide_type: str, has_metrics: bool = False, has_quote: bool = False) -> dict[str, str]:
    """
...
def get_color_for_emotion(emotion: str) -> dict[str, str]:
    """
...
def get_background_config(slide_type: str) -> dict[str, str]:
    """
...
def should_use_full_bleed(slide_index: int, total_slides: int, emotion: str) -> bool:
    """
...
def calculate_pattern_break(
    slide_index: int, total_slides: int, previous_emotion: str | None = None, current_emotion: str | None = None
) -> bool:
    """
    Determine if this slide should break the visual pattern.
    Used for emotional contrast (Duarte Sparkline technique): break at the 1/3 and 2/3
    points, or when this slide's emotion contrasts with the previous slide's.
    """
    ...
    prev = (previous_emotion or "").lower()
    cur = (current_emotion or "").lower()
    return cur in contrasting_emotions.get(prev, [])


def search_with_context(
    query: str, slide_position: int = 1, total_slides: int = 9, previous_emotion: str | None = None
) -> dict[str, Any]:
    """
    ...
    context["should_break_pattern"] = calculate_pattern_break(slide_position, total_slides, previous_emotion, emotion)
