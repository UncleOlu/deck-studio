...
def check_waterfall(idx: int, ch) -> list[str]:
    if (ch.kind != "barChart" or ch.grouping != "stacked" or ch.bar_dir == "bar"
    ...
    for i in range(1, n - 1):
        ...
        if abs(lo - level) <= tol:
            ...
            errs.append(f"ERROR slide {idx}: waterfall bar '{label}' floats at {lo:g}–{hi:g}, "
                        f"but the running total is {level:g}")
            level = hi
    if abs(tops[-1] - level) > tol or base[-1] > tol:
        errs.append(f"ERROR slide {idx}: waterfall closing total is {tops[-1]:g}, "
                    f"but the opening plus the steps gives {level:g}")
    return errs


def check_bar_baseline(idx: int, ch) -> list[str]:
    ...
    if ch.axis_min is not None and ch.axis_min > 0:
        return [f"ERROR slide {idx}: bar chart value axis starts at {ch.axis_min:g}, not 0 — bars overstate "
    ...
    if ch.axis_min is None and max(vals) > 0 and min(vals) / max(vals) > 5 / 6:
        ...
        return [f"ERROR slide {idx}: bar values {min(vals):g}–{max(vals):g} are close together, so PowerPoint's "
                "automatic axis will not start at zero; set the value-axis minimum to 0"]
    return []


def check_shares(idx: int, ch) -> list[str]:
    errs = []
...
def check_table(idx: int, rows: list[list[str]]) -> list[str]:
    ...
    for ti in total_rows:
        for c in range(1, len(rows[ti])):
            ...
            if tv is None:
                continue
            start = next((j + 1 for j in range(ti - 1, -1, -1)
                          if rows[j] and re.match(r"^\s*(sub)?total\b", rows[j][0], re.I)), 1)
            parts = [cell_value(rows[j][c]) for j in range(start, ti) if c < len(rows[j])]
            ...
            if abs(s - tv[0]) > tol:
                ...
                errs.append(f"ERROR slide {idx}: table column '{head}' parts sum to {s:g}, "
...
def clause_context(line: str, spans: list[tuple[int, int]], i: int) -> str:
    ...
    if len(ingest.label_words(" ".join(words))) < 2:
        ...
        before = line[prev:a]
        cut = list(BOUNDARY.finditer(before))
        before = before[cut[-1].end():] if cut else before
        words = before.split()[-3:] + words
    return " ".join(words + [line[a:b]])


QUALIFIERS = {"median", "mean", "average", "peer", "peers", "total", "benchmark", "target", "range", "high", "low",
              "current", "implied", "selected", "estimated"}


def bigrams(text: str) -> set[tuple[str, str]]:
    # "52-week" stays one token, so a 52-week and a 26-week average never share a phrase
    words = [ingest.SYNONYMS.get(w, w) for w in re.findall(r"\d+-[a-z]+|[a-z][a-z-]+", text.lower())
             if w not in ingest.STOP and len(w) > 2]
    return set(zip(words, words[1:]))


def check_consistency(deck) -> list[str]:
    facts = []
        ...
        for block in s.texts[1:] + [s.title]:
            for line in block.splitlines():
                ...
                for i, (v, unit, scale, a, b) in enumerate(found):
                    ...
                    facts.append(ingest.Fact(f"S{s.index}", v, unit, scale, ingest.period_of(ctx), ctx,
                                             f"slide {s.index}", f"slide {s.index}"))
    facts = [f for f in facts if f.unit or f.scale != 1.0]  # unitless integers are counts, pages, ordinals
    errs = []
    for a, b, shared in ingest.find_conflicts(facts):
        shared_phrases = {bg for bg in bigrams(a.text) & bigrams(b.text) if not set(bg) <= QUALIFIERS}
        ...
        errs.append(f"ERROR slides {a.file[6:]} and {b.file[6:]}: '{', '.join(sorted(shared))}' is "
                    f"{ingest.fmt(a)} on one and {ingest.fmt(b)} on the other — one value per metric deck-wide")
    return errs
...
def check_footnotes(s) -> list[str]:
    markers = []
    for sh in s.shapes:
    ...
    if seen != expected:
        errs.append(f"ERROR slide {s.index}: footnote markers appear as {', '.join(seen)}; "
                    "number them 1… in reading order")
