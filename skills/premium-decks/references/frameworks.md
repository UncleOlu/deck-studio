...
|---|---|---|---|---|
| What drives this number? | **Driver tree** | left-to-right tree, value on each node | Arithmetic identity holds (children multiply/add to parent) | Branches that overlap; a tree with no numbers |
...
| What should we do first? | **Prioritisation 2×2** | scatter, value vs. ease, bubbles sized by value | Both axes scored the same way for all items | Items crowded in one quadrant (the axes are wrong) |
...
4. **MECE is visible.** When a tree or bridge claims to be complete, its parts
   sum to the whole. `integrity-check.py` tests the arithmetic.
