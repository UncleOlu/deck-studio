...
def halden_inputs(d: Path, save, write) -> None:
...
def pitch(fx, src: Path) -> dict:
    C, MP, TC, PT, SP, BU = ("capitalization.xlsx", "management_projections.xlsx", "trading_comps.csv",
                             "precedent_transactions.csv", "share_price_52w.csv", "buyer_universe.csv")
    ...
    ranges = [
        ...
        (f"Trading comps ({ntm_rng[0]:.1f}x–{ntm_rng[1]:.1f}x FY26E EBITDA)",
         ps(ntm_rng[0] * ntm), ps(ntm_rng[1] * ntm)),
    ...
    return {
        ...
        "kind": "pitch",
        "meta": {"title": "Project Heron — strategic alternatives review", "author": "Advisor team",
                 "label": "Confidential · Preliminary draft"},
        "sections": ["Situation", "Valuation", "Alternatives", "Process"],
        "slides": [
            {"type": "cover", "title": "Project Heron: strategic alternatives review",
             ...
             "scripts/ingest.py."},
            {"type": "exec-summary", "title": "Key considerations",
             "points": [
            ...
            {"type": "table", "section": "Valuation",
             ...
             + " ".join(f"calc: at ${p:.2f}: premium {100 * (p / px - 1):.1f}% = {p} / {px} - 1; equity "
                        f"{g[0]:,.0f} = {p} x {sh}; EV {g[1]:,.0f} = {g[0]:,.0f} + {nd:.0f}; {g[2]:.1f}x = "
                        f"{g[1]:,.0f} / {ltm:.0f}; {g[3]:.1f}x = {g[1]:,.0f} / {ntm:.0f} [{i_px}, {i_sh}, {i_nd}, "
                        f"{i_ltm}, {i_ntm}]." for p, g in zip(cols, grid))},
            {"type": "football-field", "section": "Valuation",
             "title": f"Illustrative valuation: {above} of {len(ranges)} ranges sit above ${px:.2f}",
             "unit": "(Implied value per share, $) (1)", "min": 10, "max": 30, "step": 5,
             ...
             "footnotes": [f"Per-share values use {sh:.1f}M diluted shares and ${nd:.0f}M net debt. Comps drop the "
             ...
             + " ".join(f"calc: {lab}: ${lo:.2f} to ${hi:.2f} per share = (multiple x EBITDA - {nd:.0f}) / {sh} "
            ...
            {"type": "table", "section": "Valuation",
             ...
             + [{"cells": ["Median", f"{statistics.median(c[1] for c in comps):,.0f}",
                           f"{statistics.median(ltm_x):.1f}x", f"{statistics.median(ntm_x):.1f}x"], "kind": "summary"},
                {"cells": ["Halden at current", f"{ev_now:,.0f}", f"{ev_now / ltm:.1f}x", f"{ev_now / ntm:.1f}x"],
                 "kind": "subject"}],
             "source": f"Peer company filings; company capitalization table as of {CURRENT_DATE}",
             "notes": " ".join(f"calc: {n} {e / l_:.1f}x = {e:,.0f} / {l_:.0f} and {e / t:.1f}x = {e:,.0f} / {t:.0f} "
                               f"[{fx.span(TC)}]." for n, e, l_, t in comps)
            ...
            {"type": "options", "section": "Alternatives",
            ...
            {"type": "table", "section": "Alternatives",
             ...
             "header": ["Tier", "Buyer", "Capacity", "Rationale"], "align": ["l", "l", "r", "l"],
            ...
            {"type": "text", "section": "Process",
             ...
             "points": [
                 ...
                 {"lead": "Why.", "text": "Most valuation ranges sit above today's price, the tier-1 strategics can "
                  "pay for hub savings, and a tiered list limits disruption."},
                 {"lead": "Next steps.", "text": "Board approval of the process; advisers prepare the teaser and "
            ...
            {"type": "disclaimer", "title": "Disclaimer",
             "text": "These materials were prepared for the Board's use in reviewing strategic alternatives. They rely "
             "on public information and on projections prepared by management, which we have not verified "
