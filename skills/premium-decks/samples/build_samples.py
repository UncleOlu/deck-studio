...
def alder_vale_inputs(d: Path) -> None:
    ...
    s = wb.active
    s.title = "Summary"
    tot = [sum(h[i] for h in HOTELS) for i in (3, 4, 5, 6)]
...
class Facts:
    def __init__(self, brief: Path):
        self.rows = [json.loads(line) for line in (brief / "facts.jsonl").read_text().splitlines() if line.strip()]

    def at(self, file: str, locator: str) -> dict:
        for f in self.rows:
    ...
    def id(self, file: str, locator: str) -> str:
        return self.at(file, locator)["id"]
...
def consulting(fx: Facts) -> dict:
    ...
    hot_hk = max(seg_hk, key=seg_hk.get)
    order_hk = sorted(seg_hk, key=seg_hk.get)
    seg_en = {k: v[2] * 1000 / v[3] for k, v in seg.items()}
    order_en = sorted(seg_en, key=seg_en.get)
    rows_span = fx.span(X)
    ...
    return {
        ...
        "slides": [
            ...
            {"type": "divider", "section": "Appendix", "title": "Appendix"},
            {
                ...
                "header": ["Hotel", "Segment", "Rooms", "Housekeeping, % of rooms revenue",
                           "Energy, $ per room-night sold"],
                "align": ["l", "l", "r", "r", "r"],
                "colW": [3.0, 1.9, 1.4, 3.0, 3.0],
                "rows": [[h[0], h[1], f"{h[2]:,}", f"{100 * h[4] / h[3]:.1f}%", f"${h[5] * 1000 / h[6]:.1f}"]
                         for h in HOTELS]
                + [{"cells": ["All hotels", "", f"{sum(h[2] for h in HOTELS):,}", f"{hk_pct:.1f}%",
                              f"${en_night:.1f}"], "kind": "summary"}],
                "source": "Alder & Vale FY25 hotel P&L (hotel rows)",
                "notes": "Hotel rows [" + rows_span + "]. "
                + " ".join(f"calc: {h[0]} {100 * h[4] / h[3]:.1f}% = {h[4]:,} / {h[3]:,} and "
                           f"${h[5] * 1000 / h[6]:.1f} = {h[5]:,}k / {h[6]:,} room-nights [{rows_span}]." for h in HOTELS)
...
def banking(fx: Facts) -> dict:
    ...
    ranges = [
        ...
        (f"Trading comps ({ntm_rng[0]:.1f}x–{ntm_rng[1]:.1f}x FY26E EBITDA)", ps(ntm_rng[0] * ntm), ps(ntm_rng[1] * ntm)),
    ...
    return {
        ...
        "slides": [
            ...
            },
            {
                ...
                "series": [{"name": "Premium implied by the offer", "values": [
                    round(100 * (offer / v - 1), 1) for v in (unaff, hi_px, avg_px, lo_px)]}],
