...
def load_manifests(corpus: Path) -> list[dict]:
    rows, seen = [], set()
...
def render_pdf(pdf: Path, pages_dir: Path, out: Path) -> None:
    subprocess.run(["pdftoppm", "-r", "100", "-jpeg", "-jpegopt", "quality=80", str(pdf), str(pages_dir / "page")],
...
def contact_sheets(pages_dir: Path, pages: list[int], sheets_dir: Path) -> int:
    ...
    for s in range(0, len(pages), 4):
        ...
        cells = []
        for p in group:
            with Image.open(pages_dir / f"page-{p:03d}.jpg") as im:
                im = im.convert("RGB")
...
def main() -> int:
    ...
    for row in load_manifests(corpus):
        ...
        (out / "sample.json").write_text(json.dumps({"id": deck_id, "page_count": n, "sampled_pages": pages,
                                                     "sheets": sheets}) + "\n")
        done += 1
