...
file-by-file evidence is in `docs/history/PROVENANCE-AUDIT-v2.0.md`.

| Path | Origin | Licence | Decision |
|---|---|---|---|
| `skills/premium-decks/data/*.csv` except `slide-archetypes.csv` | nextlevelbuilder/ui-ux-pro-max-skill (commit bc826e2). v1.4 added rows to `slide-strategies.csv` and `slide-charts.csv` | MIT, © 2024 Next Level Builder, plus own rows | Keep; attributed |
| `data/slide-archetypes.csv` | Generated from the own corpus study | MIT (own) | Keep |
| `scripts/slide_search_core.py`, `scripts/search-slides.py` | Same upstream; v1.4 added the archetype domain and lint fixes | MIT, © 2024 Next Level Builder | Keep; attributed |
...
| Anthropic pptx-skill scripts (`office/**`, `add_slide.py`, `clean.py`, `thumbnail.py`) | Anthropic `pptx` skill | Proprietary: no redistribution or derivatives | **Removed in 2.0.** Kept only on the local branch `private/anthropic-pptx-scripts`, which is never pushed. Replaced by own `deck_edit.py`, `validate_pptx.py`, `deck_thumbnails.py` |
| `references/pptx-mode.md` (pptxgenjs mechanics, fonts, QA prose) and `references/quality-floor.md` (banned list, Required 2/3/5/10) | Passages that followed the Anthropic pptx skill's wording and order | — | **Rewritten in 2.0** in new wording and grouping; values replaced with the kit's own scale and grid |
...
| `references/design-dna.md` | Directions modelled on public product sites; prose rewritten in 2.0 in own words (the earlier text cited superdesign.dev, licence unknown) | MIT (own wording); colour values and font names are facts | Keep; product names used descriptively |
